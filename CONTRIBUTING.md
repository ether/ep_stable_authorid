# Contributing to ep_stable_authorid

Thanks for helping improve `ep_stable_authorid` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_stable_authorid` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_stable_authorid from your local clone
pnpm run plugins i --path ../ep_stable_authorid

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_stable_authorid/` are picked up after a server restart.
...
* Etherpad version, ep_stable_authorid version, browser + OS
* Steps to reproduce
