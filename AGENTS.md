# Agent Guide — ep_stable_authorid
...
`ep_stable_authorid` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_stable_authorid`:
...
* Server: `getAuthorId`, `init_ep_stable_authorid`, `loadSettings`
